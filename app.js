...
game.evolvePokemon = () => {
  for (let i = 0; i < game.party.length; i++) {
    if(game.party[i].number < pokemon.length - 1) {
...
*/
console.log(`The name of each Pokemon in your party:`);
for (let num in game.party) {
  console.log(`- ${game.party[num].name}`)
}
...
console.log(`The starter Pokemon are:`);

pokemon.forEach((indx) => {
  if (indx.starter) {
...
game.catchPokemon = (pokemonObj) => game.isIncludeParty(pokemonObj.number) 
? `Pokemon is already exist in the party` : game.party.push(pokemonObj);
  
game.catchPokemon(pokemon[70]);
console.log(game.party);
console.log(`==================`);
